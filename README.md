...
1.  **Separation**: Uses `audio-separator` with the **BS-Roformer-Viperx-1297** model to split audio into Vocals and Background (Instrumental). The **Lossless Background** method (Original - Vocals) ensures ambient sounds like birds and nature are preserved 100%.
...
4.  **Smart Sync**: Automatically calculates and corrects any latency/drift (up to <1ms precision) using **Cross-Correlation** alignment.
...
subgraph Parallel ["Steps 2 & 3 — Processing"]
    direction LR
...
subgraph Sync ["Step 4 — Smart Sync"]
    VE --> SYNC["Smart Sync<br/>(Cross-Correlation)"]:::processing
    MD --> SYNC
end

subgraph Mix ["Step 5 — Final Mix"]
    SYNC --> MIX["FFmpeg Mix"]:::processing
...
style Parallel fill:none,stroke:#526350,stroke-width:1px,stroke-dasharray: 4,opacity:0.6
