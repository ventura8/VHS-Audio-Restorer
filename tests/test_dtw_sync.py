...
def test_parallel_dtw_logic():
    ...
    with patch("concurrent.futures.ProcessPoolExecutor") as MockExecutor, \
        ...
        with patch("restore_audio_hybrid.CPU_THREADS", 4), \
                patch("restore_audio_hybrid.DTW_RESOLUTION", 40), \
                patch("restore_audio_hybrid.GPU_VRAM_GB", 0):
            ...
            restore_audio_hybrid.librosa = mock_librosa
            restore_audio_hybrid.fastdtw = MagicMock()

            # But _align_stems_dtw is what we test
            # It calls sf.read/write at the end. Patch them.
            with patch("restore_audio_hybrid.sf.read", return_value=(np.zeros((N * 100, 2)), 44100)), \
                    patch("restore_audio_hybrid.sf.write"):

                restore_audio_hybrid._align_stems_dtw(ref, proc, out)
...
def test_gpu_dtw_worker():
    ...
    with patch.dict("sys.modules", {"torch": MagicMock(), "librosa": MagicMock()}):
        ...
        result_path = restore_audio_hybrid._run_gpu_dtw_chunk(args)
...
def test_align_stems_dtw_dispatches_gpu():
    ...
    with patch("restore_audio_hybrid.torch") as mock_torch, \
            patch("restore_audio_hybrid.GPU_VRAM_GB", 10), \
            ...
            patch("restore_audio_hybrid.librosa") as mock_librosa, \
            patch("restore_audio_hybrid.sf.read", return_value=(np.zeros((1000, 2)), 44100)), \
            patch("restore_audio_hybrid.sf.write"):
