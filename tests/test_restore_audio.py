...
def test_is_valid_audio_robust(tmp_path):
    ...
    with patch("restore_audio_hybrid.sf.SoundFile") as mock_sf:
        mock_sf.return_value.__enter__.return_value.frames = 10
...
def test_process_skip(mock_valid, tmp_path):
...
@patch("restore_audio_hybrid._denoise_music_step")
...
@patch("restore_audio_hybrid.attempt_run_with_retry")
@patch("restore_audio_hybrid.is_valid_audio", return_value=False)
def test_denoise_music_step(mock_valid, mock_retry, tmp_path):
    """Test music denoising step."""
    music = tmp_path / "music.wav"
    music.write_text("music data")
    out_dir = tmp_path / "denoised"
    out_dir.mkdir()

    # Create expected output
    denoised = out_dir / "music_(No Noise).wav"
    denoised.write_text("denoised")

    # Mock success and verify command builder
    def side_effect(builder, bs, description):
        cmd = builder(bs)
    ...
    result = restore_audio_hybrid._denoise_music_step(music, out_dir)
    assert "No Noise" in result.name


@patch("restore_audio_hybrid.attempt_cpu_run_with_retry")
@patch("restore_audio_hybrid.get_audio_duration_sec", return_value=60.0)
def test_final_mix_step(mock_dur, mock_retry, tmp_path):
    """Test final mix step."""
    video = tmp_path / "video.mp4"
    ...
    restore_audio_hybrid._final_mix_step(video, vocals, music, output)
    mock_retry.assert_called_once()
...
@patch("restore_audio_hybrid.attempt_run_with_retry")
@patch("restore_audio_hybrid.is_valid_audio", return_value=False)
def test_denoise_music_step_no_noise_selection(mock_valid, mock_retry, tmp_path):
    ...
    regular = out_dir / "music_(Instrumental)_denoised.wav"
    regular.write_text("regular")
    no_noise = out_dir / "music_(Instrumental)_(No Noise).wav"
...
@patch("restore_audio_hybrid.attempt_run_with_retry")
@patch("restore_audio_hybrid.is_valid_audio", return_value=False)
def test_denoise_music_step_fallback_to_regular(mock_valid, mock_retry, tmp_path):
...
def test_denoise_music_step_fallback_to_raw(mock_log, mock_valid, mock_retry, tmp_path):
    """Test denoise step falls back to raw music when denoising fails."""
    music = tmp_path / "music.wav"
    music.write_text("music data")
    out_dir = tmp_path / "denoised"
    out_dir.mkdir()
...
def test_separate_stems_step_skip(mock_valid, mock_retry, tmp_path):
    ...
    result = restore_audio_hybrid._separate_stems_step(audio, out_dir)
    mock_retry.assert_not_called()
    assert result == (vocal_file, back_file)


@patch("restore_audio_hybrid.attempt_run_with_retry")
@patch("restore_audio_hybrid.is_valid_audio", side_effect=[True])
def test_denoise_music_step_skip(mock_valid, mock_retry, tmp_path):
    ...
    existing = out_dir / "music_(Instrumental)_denoised.wav"
...
def test_format_time_negative():
    ...
    assert restore_audio_hybrid.format_time(-10) == "00:00:00.00"
...
def test_run_command_with_progress_adds_to_active(mock_popen):
    ...
    def wait_side_effect():
        assert mock_proc in restore_audio_hybrid._active_processes
        return 0

    mock_proc.wait.side_effect = wait_side_effect

    restore_audio_hybrid.run_command_with_progress(["echo"], description="Test")
    assert mock_proc not in restore_audio_hybrid._active_processes
